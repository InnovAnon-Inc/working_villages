...
local function put_func(_,stack)
	if landscaping_demands[stack:get_name()] then
		return false
	end
	return true;
end
local function take_func(villager,stack)
	...
	if landscaping_demands[item_name] then
...
local function landscaping_job(self)
	self:handle_night()
	...
	if self:timer_exceeded("landscaper:change_dir",50) then
...
working_villages.register_job("working_villages:job_landscaper", {
	...
	jobfunc = landscaping_job,
})

working_villages.foundation_nodes = foundation_nodes
working_villages.landscaping_job = landscaping_job
